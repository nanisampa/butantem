function ContactInfo({ project, noClamp }) {
    return (
        <div className="mb-2">
            {project.direccion ? <p className={`py-1 text-gray-800 ${!noClamp ? ' box clamp-1 orient-vertical overflow-hidden' : null} `}><a href={`http://maps.google.com/?q=${project.direccion.replace(/ /g, '').replace(/#/g, '')}`} className="text-gray-800 hover:underline "><span className="font-medium text-gray-800">📍 Endereço: </span>{project.direccion}</a></p> : null}
